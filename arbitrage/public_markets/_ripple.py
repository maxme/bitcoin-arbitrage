...
class Ripple(Market):
    def __init__(self, currency, code):
        ...
        self.code = code  # the counter currency issuer to BTC, e.g. "rvYAfWj5gh67oV6fW32ZzP3Aw4Eubs59B" (Bitstamp)
        self.update_rate = 20

    def update_depth(self):
        url = config.ripple_rippled
        biddata = '{ "method" : "book_offers", "params" : [ { "taker_pays" : { "currency" : "BTC", "issuer" : "' + config.ripple_BTC_issuer + '" }, "taker_gets" : { "currency" : "' + self.currency + '", "issuer" : "' + self.code + '" } } ] }'
        biddata = biddata.encode('ascii')
        bidreq = urllib.request.Request(url, biddata,
                                     headers={
        ...
        askdata = '{ "method" : "book_offers", "params" : [ { "taker_gets" : { "currency" : "BTC", "issuer" : "' + config.ripple_BTC_issuer + '" }, "taker_pays" : { "currency" : "' + self.currency + '", "issuer" : "' + self.code + '" } } ] }'
        askdata = askdata.encode('ascii')
        askreq = urllib.request.Request(url, askdata,
                                     headers={
            "Content-Type": "application/x-www-form-urlencoded",
            "Accept": "*/*",
            "User-Agent": "curl/7.24.0 (x86_64-apple-darwin12.0)"})
